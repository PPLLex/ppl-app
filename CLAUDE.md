...
- **Logo pipeline (auto-sync).** The PPL logo lives as a static asset at `packages/frontend/public/ppl-logo.webp` (43KB optimized WebP). Every Vercel build runs `packages/frontend/scripts/fetch-logo.mjs` as a prebuild step, pulling the latest logo from the backend API (`/api/settings/branding`) and rewriting `ppl-logo.webp` with a fresh optimized copy. When an admin uploads a new logo via `POST /api/settings/branding/logo`, the backend saves to DB AND fires the Vercel deploy hook (`VERCEL_DEPLOY_HOOK_URL` env var). Result: logo changes auto-propagate to the static asset in 60-90s, no manual steps. The hook URL is named `logo-sync` in the Vercel project Settings → Git → Deploy Hooks, and stored as `VERCEL_DEPLOY_HOOK_URL` in Railway's ppl-app service env vars.
